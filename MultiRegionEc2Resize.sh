...
check_desire_state(){
  ...
  current_instance_type=$(aws ec2 describe-instances --region $instance_region --instance-id $instance_id --query "Reservations[*].Instances[*].InstanceType" --output text)
...
stop_instance () {
  ...
  instance_state=$(aws ec2 describe-instances --region $instance_region --instance-ids $instance_id --query Reservations[*].Instances[*].State.Name --output text)
  echo "INFO: Instance state is: $instance_state"
      if [ $instance_state = "running" ]; then
          echo "INFO: Stopping instance $instance_id"
          aws ec2 stop-instances --region $instance_region --instance-ids $instance_id
      fi
      i=0
      ...
      until [ $state = "stopped" ]; do
          sleep 1s
          i=$((i+1))
          state=$(aws ec2 describe-instances --region $instance_region --instance-ids $instance_id --query Reservations[*].Instances[*].State.Name --output text)
          printf "."
          if [ "$i" -ge "180" ]; then
              printf "\n"
    ...
    create_snapshots(){
      aws ec2 describe-volumes --region $instance_region --filter Name=attachment.instance-id,Values=$instance_id --query Volumes[*].{ID:VolumeId} --output text | tr '\t' '\n' > `pwd`/volumesList 2>&1
      ...
      do
         #Create a decription for the snapshot that describes the volume: servername.device-backup-date
          ...
          snapresult=$(aws ec2 create-snapshot --region $instance_region --output=text --description $description --volume-id $volume_id --query SnapshotId)
          # Add some tags to the snapshot
          tagresult=$(aws ec2 create-tags --region $instance_region --resource $snapresult --tags Key=CreatedBy,Value=ChangeInstanceType)
    ...
    change_instance_type () {
      ...
      instance_state=$(aws ec2 describe-instances --region $instance_region --instance-ids $instance_id --query Reservations[*].Instances[*].State.Name --output text)
          if [ $instance_state == "stopped" ]; then
               ...
               aws ec2 modify-instance-attribute --region $instance_region --instance-id $instance_id --instance-type "{\"Value\": \"$instance_type\"}"
          else
  ...
  ## Start Ec2 instance
    start_instance () {
      ...
      new_instance_type=$(aws ec2 describe-instance-attribute --region $instance_region --instance-id $instance_id --attribute instanceType)
      printf "\n"
...
## Main
change_instance (){
  temp_array="$(cat instanceID.txt | sed 's/,/ /g' | cut -d' ' -f 1,2,3)"
  #echo "$temp_array"
  instances_array=($temp_array)
  #echo "${tag_array[*]}"
  a=0;
  length=$(( ${#instances_array[*]}-1 ))
  for var in `seq 0 3 $length`; do
     #echo "${instances_array[var]} ${instances_array[var+1]} ${instances_array[var+2]}";
     instance_region="${instances_array[var]}"
     instance_id="${instances_array[var+1]}"
     instance_type="${instances_array[var+2]}"
     check_desire_state
 done
